...
CREATE INDEX idx_blog_author_user ON blog_posts(author_user_id);
...
CREATE TABLE package_departures(
                                   ...
                                   seats_available INTEGER CHECK (
                                       seats_available IS NULL OR
                                       (seats_total IS NOT NULL AND seats_available >= 0 AND seats_available <= seats_total)
                                       ),
                                   price_cents_override INTEGER CHECK (price_cents_override IS NULL OR price_cents_override >= 0),
                                   created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
                                   updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
                                   CONSTRAINT chk_dep_dates CHECK (end_date >= start_date)
);
...
CREATE TABLE bookings(
    ...
    currency CHAR(3) NOT NULL DEFAULT 'USD', --TODO will have to adjust this depending on userAccount location
...
CREATE TABLE booking_items(
                              ...
                              package_departure_id BIGINT REFERENCES package_departures(id) ON DELETE SET NULL, -- ← removed NOT NULL
